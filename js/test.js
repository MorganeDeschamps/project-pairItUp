...
class CardsTest {
    constructor() {
        this.array = symbolsTest;
        this.cards = new Cards();
        this.cardOne = [1,2,3,4,5,6,7,8,9];
        this.cardTwo = [1,2,3,4,5,6,7,8,9];
        this.emptyCard = [];
        
    }

    filter(origin, cardToBuild, previous) {
        return origin.filter(el => !cardToBuild.includes(el) && !previous.includes(el))
    }
    ...
    cardRandomTest(cardToBuild, previous) {
        let selectFrom = this.array.filter(el => !previous.includes(el))
        cardToBuild.forEach((symbol, index) => {
            selectFrom = this.filter(selectFrom, cardToBuild, previous);
            cardToBuild.splice(index, 1, this.randomEl(selectFrom))
        })
        return cardToBuild;
    }

    addCommon(cardToBuild, previous) {
        cardToBuild.splice(this.randomEl(cardToBuild), 1, this.randomEl(previous));
        return cardToBuild;
    }
    ...
    buildCard(which) {
        switch(which) {
            case "main":
                this.cardRandomTest(this.cardOne, this.emptyCard)
                break;
            case "player":
                this.cardRandomTest(this.cardTwo, this.emptyCard)
                break;
...
class TestTwo {
    ...
    filterOnce(origin, current) {
        return origin.filter(symbol => !current.includes(symbol));
    }

    filterTwice(origin, current, previous) {
        return origin.filter(el => !current.includes(el) && !previous.includes(el))
    }
    ...
    cardRandom(cardToBuild, cardAbove) {
        if (!cardAbove) {
            cardToBuild.forEach((symbol, index) => {
                let selectFrom = this.filterOnce(this.array, cardToBuild);
                cardToBuild.splice(index, 1, this.randomFrom(selectFrom))
            })
            return cardToBuild;
        } else {
            let selectFrom = this.filterOnce(this.array, cardAbove);

            cardToBuild.forEach((symbol, index) => {
                cardToBuild.splice(index, 1, this.randomFrom(selectFrom))
                selectFrom = this.filterTwice(selectFrom, cardToBuild, cardAbove);
            })
            cardToBuild.splice(this.random(8), 0, this.randomFrom(cardAbove));
            return cardToBuild;
        }
    }
...
/*       while(cardToBuild.length < 9) {
            cardToBuild.push(this.randomEl(selectFrom))
            selectFrom = this.filter(selectFrom, cardToBuild, previous);
        }
*/